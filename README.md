...
> Write prompts, workflows, and logs as one executable text with explicit time and metadata.
...
- the ability to run ```zyng fenced code blocks inside Markdown files  
...
## Scope: what Zyng is / is not

Zyng is:

- a **timeline-first DSL** for LLM workflows and logs  
- a way to write prompts, workflows, and logs as **one executable text**  
- a front-end language that sits **in front of** Python and LLM APIs

Zyng is **not**:

- a general-purpose programming language competing with Python or Rust  
- a high-performance runtime or type system  
- a replacement for LLM backends (OpenAI, Claude, Gemini, local models, etc.)

Heavy computation and application logic stay in Python or other languages.  
Zyng focuses on the **text layer**: timelines, metadata, and executable documentation.
...
### v0.1 – Core language & README runner

- Define and stabilize Core syntax for:
  - `show`
  - `read`
  - `write`
  - `use`
...
- Keep the Core small; push complex logic to Python
...
- Start treating the Core spec as mostly backward-compatible

---

## Status & feedback

Zyng is in a very early experimental stage (`v0.0.1`).  
Feedback, ideas, and discussion about the language design and runtime architecture are very welcome.
