...
### 4-1. Known Limitations (v0.0.1 の既知の制約)

- **文字列中の `:::` 誤検知**
  - 現状の簡易パーサは `line.split(":::", 1)` による素朴な分割を行っているため、文字列リテラルの中に `:::` が含まれる場合（例: `show "Hello ::: World"`）に正しくパースできない。
  - v0.1 で最小限の字句解析器（Lexer）を導入し、クォート内とメタ情報部分を明確に区別して扱う予定。

- **State（状態）の欠如**
  - 現在のランナーは「1 行読んでその場で実行して捨てる」方式のため、変数を保持するメモリ（Context）が存在しない。
  - `let` や `read` を含む Core 言語を実装するためには、`Statement` と `Context` を持つ実行モデルへの刷新（パースフェーズと実行フェーズの分離）が必要。
